...
lg = bs.login()
rs = bs.query_history_k_data_plus("sh.601375",
...
#     if olddictvalue<0 and oldTwokk>olddictvalue and kk<=0 and oldKK>0:
#         ax2.scatter(kX, kk, color="w", linewidth=0.0004)
#     oldKK=kk
...
#     #一阶导数小于0，二阶导数小于0,一阶导数小于二阶导数，二阶导数递增
...
for i in range(len(erjieK)):
    ...
    if onek<=0 and twok<=0 and twok>onek and oldTwok<onek:
        ...
        if i>6:
            ...
            yesToday3TwoK=erjieK[i-3]
            yesToday4TwoK=erjieK[i-4]
            yesToday1oneK=yijiedict.get(currentx-1)
            yesToday2oneK=yijiedict.get(currentx-2)
            yesToday3oneK=yijiedict.get(currentx-3)
            yesToday4oneK=yijiedict.get(currentx-4)
            if yesToday1TwoK[1]<yesToday1oneK and yesToday2TwoK[1]<yesToday2oneK and yesToday3TwoK[1]<yesToday3oneK and yesToday4TwoK[1]<yesToday4oneK:
                print("======一阶========"+str(i)+"===============")
                print(yesToday1oneK)
                print(yesToday2oneK)
                print(yesToday3oneK)
